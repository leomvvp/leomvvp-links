...
async function getDiscordData() {
    try {
        ...
        discordTag.textContent = `@${user.discord_user.username}`;
        ...
        const game = user.activities.find(
        ...
        if (game) {
            ...
            if (game.assets?.large_image) {
                ...
                gameIcon.src = "assets/icons/game.png";
        ...
        username.textContent = "Sin conexión";
...
setInterval(() => {
    ...
    if (spotifyStart && spotifyEnd) {
        ...
        const current = Date.now() - spotifyStart;
        const total = spotifyEnd - spotifyStart;

        if (current < total) {

            spotifyCurrent.textContent = formatTime(current);
            spotifyTotal.textContent = formatTime(total);
...
setInterval(getDiscordData, 3000);